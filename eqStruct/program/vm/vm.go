...
import (
	...
	"github.com/nomoresecretz/ghoeq-common/eqStruct/program/compiler"
...
func (vm *VM) Run(s eqStruct.EQStruct, b []byte) (int, error) {
	var ip, bp int
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpCall:
			...
			err := vm.executeCall(s, b, str, int(numArgs))
			if err != nil {
				return bp, err
			}
		case code.OpRetVal:
			...
			frame := vm.popFrame()
			vm.stackPointer = frame.basePtr - 1
		...
		case code.OpGetStructAction:
			index := code.ReadUint8(ins[ip+1:])
			vm.currentFrame().ip += 1

			def := object.StructActions[index]

			err := vm.push(def.StructAction)
			if err != nil {
				return bp, err
			}
		case code.OpClosure:
...
func (vm *VM) callClosure(cl *object.Closure, numArgs int) error {
	...
	vm.stackPointer = frame.basePtr + cl.Fn.NumLocals
...
func (vm *VM) executeCall(s eqStruct.EQStruct, b []byte, r reflect.Value, numArgs int) error {
	...
	case *object.StructAction:
		return vm.callStructAction(callee, s, r, b, numArgs)
	default:
		return fmt.Errorf("calling non-function and non-built-in %T", callee)
	}
}

func (vm *VM) callStructAction(eqAction *object.StructAction, s eqStruct.EQStruct, r reflect.Value, b []byte, numArgs int) error {
	args := vm.stack[vm.stackPointer-numArgs : vm.stackPointer]

	res := eqAction.Fn(s, r, b, args...)
	vm.stackPointer = vm.stackPointer - numArgs - 1
